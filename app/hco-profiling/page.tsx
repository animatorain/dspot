...
const TOP_N_NEW_HCO = 10

export default function HcoProfilingPage() {
  const searchParams = useSearchParams()
  const hcoFilterParam = searchParams.get("hco")
  const router = useRouter()

  const [selectedYear, setSelectedYear] = useState("2025")
  const [selectedHco, setSelectedHco] = useState<string>("all")
  const [hcoStatusFilter, setHcoStatusFilter] = useState<string>("All")

  // Apply HCO filter from URL
  useEffect(() => {
    if (hcoFilterParam) {
      setSelectedHco(hcoFilterParam)
    }
  }, [hcoFilterParam])

  const yearNumbers = useMemo(() => (selectedYear ? [Number.parseInt(selectedYear)] : []), [selectedYear])
  ...
  const aggregatedHcoData = useMemo(() => {
    ...
    console.log("[v0] Filtered records by year:", filteredRecords.length)
    ...
    const result = Array.from(grouped.entries()).map(([hcoName, data]) => ({
      hcoName,
      hcoId: data.hcoId,
    ...
    console.log("[v0] Aggregated HCO data:", result.length)
    return result
  }, [selectedYear])

  // Filter by HCO category
  const categoryFilteredData = useMemo(() => {
    const result = aggregatedHcoData.filter((r) => HCO_CATEGORIES.includes(r.category))
    console.log("[v0] Category filtered data:", result.length)
    return result
  }, [aggregatedHcoData])

  // Filter by HCO status
  const filteredHcoData = useMemo(() => {
    console.log("[v0] HCO Status Filter:", hcoStatusFilter)
    ...
    return filteredHcoData
      ...
      .map((r) => {
        ...
        const buBreakdown = hcoBuYearSummary
          .filter((bu) => bu.hcoName === r.hcoName && selectedYear && selectedYear === bu.year.toString())
          .reduce(
            (acc, bu) => {
  ...
  const newHighSpendingHcos = useMemo(() => {
    // Get all HCO IDs that exist in 2024
    const hcosIn2024 = new Set(hcoYearSummary.filter((r) => r.year === 2024).map((r) => r.hcoId))

    // Get HCOs only in 2025 (not in 2024)
    const newHcosIn2025 = hcoYearSummary
      .filter((r) => r.year === 2025 && !hcosIn2024.has(r.hcoId))
      .sort((a, b) => b.totalAmount - a.totalAmount)
      .slice(0, TOP_N_NEW_HCO)

    // Add BU usage details
    return newHcosIn2025.map((hco, index) => {
      const buUsage = hcoBuYearSummary
        .filter((bu) => bu.hcoId === hco.hcoId && bu.year === 2025)
        .sort((a, b) => b.activityCount - a.activityCount)
        .map((bu) => `${bu.bu} (${bu.activityCount})`)
        .join(", ")
      ...
      return {
        ...
        totalAmount: hco.totalAmount,
        activityCount: hco.activityCount,
        buUsage: buUsage || "–",
      }
    })
  }, [])

  const lowFrequencyHcos = useMemo(() => {
    // Aggregate all activities across all years for each HCO
    const hcoTotalActivities = new Map<
      string,
      { hcoId: string; hcoName: string; totalActivities: number; totalAmount: number; category: string }
    >()

    hcoYearSummary.forEach((r) => {
      const existing = hcoTotalActivities.get(r.hcoId)
      if (existing) {
        existing.totalActivities += r.activityCount
        existing.totalAmount += r.totalAmount
      } else {
        hcoTotalActivities.set(r.hcoId, {
          hcoId: r.hcoId,
          hcoName: r.hcoName,
          totalActivities: r.activityCount,
          totalAmount: r.totalAmount,
          category: r.category,
        })
      }
    ...
    // Get all HCOs, sorted by activity count (ascending - from low to high)
    const sorted = Array.from(hcoTotalActivities.values()).sort((a, b) => a.totalActivities - b.totalActivities)

    // Add BU usage details
    return sorted.map((hco) => {
      const buUsage = hcoBuYearSummary
        .filter((bu) => bu.hcoId === hco.hcoId)
        .reduce(
          (acc, bu) => {
            const existing = acc.find((item) => item.bu === bu.bu)
            if (existing) {
              ...
              acc.push({ bu: bu.bu, activityCount: bu.activityCount })
            }
            return acc
          },
          [] as Array<{ bu: string; activityCount: number }>,
        )
        .sort((a, b) => b.activityCount - a.activityCount)
        .map((bu) => `${bu.bu} (${bu.activityCount})`)
        .join(", ")
      ...
      return {
        hcoName: hco.hcoName,
        totalActivities: hco.totalActivities,
        totalAmount: hco.totalAmount,
        buUsage: buUsage || "–",
      }
    })
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#F5F7FB" }}>
      <header className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-[1600px] mx-auto px-8 py-6">
          <h1 className="text-2xl font-bold text-gray-900">HCO Profiling Overview</h1>
          <p className="text-sm text-gray-500 mt-1">
            Comprehensive HCO lifecycle analysis with year-over-year comparison and BU engagement details
          </p>
          {selectedHco !== "all" && (
            <div className="mt-3 flex items-center gap-2">
              ...
              <button onClick={() => setSelectedHco("all")} className="text-sm text-gray-500 hover:text-gray-700 ml-2">
                Clear ✕
              </button>
            </div>
          )}
        ...
        <div className="max-w-[1600px] mx-auto px-8 py-4">
          <div className="flex items-center gap-6 flex-wrap">
            ...
            <MultiSelect
              label="Year"
              options={ALL_YEARS}
              selected={yearNumbers.map(String)}
              onChange={(years) => setSelectedYear(years[0] || "2025")}
      ...
      <main className="max-w-[1600px] mx-auto px-8 py-6">
        ...
        <section className="mb-6">
          ...
          <SectionCard title="Side-by-Side Year Comparison" subtitle="Highest spending organizations by year">
        ...
        <section className="mb-6">
          <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
            ...
            New High-Spending HCOs in 2025
          </h2>
          <SectionCard
            title={`Top ${TOP_N_NEW_HCO} New HCOs in 2025`}
            subtitle="HCOs that only appeared in 2025 (no 2024 record), sorted by total spending"
          >
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-semibold text-gray-700">HCO Name</th>
                    <th className="px-4 py-3 text-left font-semibold text-gray-700">Category</th>
                    <th className="px-4 py-3 text-right font-semibold text-gray-700">2025 Total Spending</th>
                    <th className="px-4 py-3 text-right font-semibold text-gray-700">2025 Activities</th>
                    <th className="px-4 py-3 text-left font-semibold text-gray-700">BU Usage</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {newHighSpendingHcos.length > 0 ? (
                    newHighSpendingHcos.map((hco) => (
                      <tr key={hco.rank} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 text-gray-600">{hco.rank}</td>
                        <td className="px-4 py-3 font-medium text-gray-900">{hco.hcoName}</td>
                        <td className="px-4 py-3 text-gray-600">{hco.category}</td>
                        <td className="px-4 py-3 text-right font-semibold text-[#005EB8]">
                          {formatCurrency(hco.totalAmount)}
                        </td>
                        <td className="px-4 py-3 text-right text-gray-600">{hco.activityCount}</td>
                        <td className="px-4 py-3 text-gray-600 text-sm">{hco.buUsage}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan={6} className="px-4 py-8 text-center text-gray-500 italic">
                        No new HCOs found in 2025
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <section className="mb-6">
          ...
          <SectionCard
            title="All HCOs by Activity Frequency"
            subtitle="All HCOs sorted by total activities (from low to high)"
          >
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 py-3 text-left font-semibold text-gray-700">HCO Name</th>
                    <th className="px-4 py-3 text-right font-semibold text-gray-700">Total Activities</th>
                    <th className="px-4 py-3 text-right font-semibold text-gray-700">Total Spending</th>
                    <th className="px-4 py-3 text-left font-semibold text-gray-700">BU Usage</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {lowFrequencyHcos.length > 0 ? (
                    lowFrequencyHcos.map((hco, index) => (
                      <tr key={index} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 font-medium text-gray-900">{hco.hcoName}</td>
                        <td className="px-4 py-3 text-right text-gray-600">{hco.totalActivities}</td>
                        <td className="px-4 py-3 text-right font-semibold text-[#005EB8]">
                          {formatCurrency(hco.totalAmount)}
                        </td>
                        <td className="px-4 py-3 text-gray-600 text-sm">{hco.buUsage}</td>
      ...
      <footer className="py-4 text-center text-xs text-gray-400">
        HCO Profiling – Enhanced with lifecycle analysis, BU engagement, and low-frequency HCO insights
      </footer>
    </div>
  )
}
